...
def get_council_id(conn, council_name:str, council_table_name:str) -> int:
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT council_id FROM {council_table_name} WHERE council_name ILIKE %s", (council_name,))
            result = cursor.fetchone()
            if result:
                return result[0]
            else:
                logging.error(f"Council name '{council_name}' not found in the database.")
                raise ValueError(f"Council name '{council_name}' not found in the database.")
    except Exception as e:
...
def get_status_type_id(conn, status_name:str, status_type_table_name:str) -> int:
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT status_type_id FROM {status_type_table_name} WHERE status_type ILIKE %s", (status_name,))
            result = cursor.fetchone()
            if result:
                return result[0]
            else:
                logging.error(f"Status type '{status_name}' not found in the database.")
                raise ValueError(f"Status type '{status_name}' not found in the database.")
    except Exception as e:
...
def get_application_type_id(conn, application_type_name:str, application_type_table_name:str) -> int:
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT application_type_id FROM {application_type_table_name} WHERE application_type ILIKE %s", (application_type_name,))
            result = cursor.fetchone()
            if result:
                return result[0]
            else:
                logging.error(f"Application type '{application_type_name}' not found in the database.")
                raise ValueError(f"Application type '{application_type_name}' not found in the database.")
    except Exception as e:
...
def load_application_to_rds(conn, table_name:str, application_data: dict, council_id:int, status_type_id:int, application_type_id:int) -> int:
    ...
    try:
        with conn.cursor() as cursor:
            ...
            conn.commit()
            logging.info(f"Successfully inserted application data into RDS with application_id: {application_id}.")
...
def load_list_of_pdfs_to_s3(s3_client, bucket_name:str, file_paths:list, council_name:str, application_number:str):
    ...
    try:
        for file_path in file_paths:
            s3_key = generate_s3_key(council_name, application_number, os.path.basename(file_path))
...
def get_document_type_id(conn, document_type_name:str, document_type_table_name:str) -> int:
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT document_type_id FROM {document_type_table_name} WHERE document_type ILIKE %s", (document_type_name,))
            result = cursor.fetchone()
            if result:
                return result[0]
            else:
                logging.error(f"Document type '{document_type_name}' not found in the database.")
                raise ValueError(f"Document type '{document_type_name}' not found in the database.")
    except Exception as e:
...
def load_document_metadata_to_rds(conn, table_name:str, application_id:int, s3_object_key:str, document_type_id:int):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            conn.commit()
            logging.info(f"Successfully inserted document metadata into RDS for application_id: {application_id}.")
    except Exception as e:
...
def load_application_data_and_pdfs(conn, council_name: str, application_info: dict, s3_client, bucket_name:str):
    ...
    council_id = get_council_id(conn, council_name, os.getenv('COUNCIL_TABLE_NAME'))
    status_type_id = get_status_type_id(conn, application_info['status_type'], os.getenv('STATUS_TYPE_TABLE_NAME'))
    application_type_id = get_application_type_id(conn, application_info['application_type'], os.getenv('APPLICATION_TYPE_TABLE_NAME'))

    # Load application data to RDS
    application_id = load_application_to_rds(conn, os.getenv('APPLICATION_TABLE_NAME'), application_info, council_id, status_type_id, application_type_id)

    # Load PDFs to S3 and document metadata to RDS
    for document in application_info['documents']:
        s3_key = generate_s3_key(council_name, application_info['application_number'], os.path.basename(document['file_path']))
        load_pdf_to_s3(s3_client, bucket_name, document['file_path'], s3_key)
        document_type_id = get_document_type_id(conn, document['document_type'], os.getenv('DOCUMENT_TYPE_TABLE_NAME'))
        load_document_metadata_to_rds(conn, os.getenv('DOCUMENT_TABLE_NAME'), application_id, s3_key, document_type_id)
