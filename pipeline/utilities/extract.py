...
def prime_weekly_decided_state(session: requests.Session) -> bool:
    """
    Establishes server-side search state for the weekly decided list.

    Always selects the most recent available week. After a successful call,
    ``pagedSearchResults.do`` will return decided applications for that week.
    """
    logger.info("Priming server state for weekly decided list...")
...
def _reprime_current_list(session: requests.Session) -> bool:
    """Re-primes the session state for current applications."""
    return prime_session_state(session)


def _reprime_weekly_list(session: requests.Session) -> bool:
    """Re-primes the session state for weekly decided applications."""
...
def get_current_applications(session: requests.Session) -> List[Dict[str, str]]:
    ...
    return paginate_applications_helper(session, _reprime_current_list, 1)
...
def get_weekly_decided_applications(
    ...
    if not prime_weekly_decided_state(session):
        logger.error(
            "Failed to prime weekly decided list server state. Exiting.")
        return []

    # page_limit = 1 remove after testing
    return paginate_applications_helper(session, _reprime_weekly_list, 1)
...
def filter_new_applications(
    ...
    logger.info("Found %d new applications", len(new_apps))
    return new_apps
...
def _run_scraper_pipeline(
    ...
    logger.info("Completed scrape: %s. Total to process: %d",
                label, len(result))
    return result
...
if __name__ == "__main__":
    print(run_scraper_current_applications(None)[0])
    print(run_scraper_weekly_applications(None)[0])
